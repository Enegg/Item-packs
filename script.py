...
def garbage():
    ...
    what_to_ask_for = {
        ...
        "WEAPON": (
            [physical, explosive, electric],
            6,
            7,
            *((8, 9, 10) if mode == 0 else ()),
            11,
            14,
            *((15,) if mode == 0 else ()),
            cost,
    ...
    while True:
        ...
        elem = (
            input(f'{"[" + last_elem + "] " if last_elem else ""}Element [PHEC]: ')[:1].lower() or elem
        )
        ...
        if mode:
            ...
            tr = (
                input(
                    f'{"[" + last_tier + "] " if last_tier else ""}Transform range [CRELMD][ CRELMD]: '
                )
                .replace("-", "")
                .upper()
                or tr
        ...
        for key in stats_to_ask_for:
            if isinstance(key, int):
                ...
                if not is_divine or stat not in divineable:
                    continue
            ...
            if isinstance(key, list):
                if item_elem == "COMBINED":
                    key = tuple(key)
                else:
                    key = (
                        key[("PHYSICAL", "EXPLOSIVE", "ELECTRIC").index(item_elem)]
                        if item_elem
                        else "PHYSICAL"
                    )
            ...
            if isinstance(key, tuple):
                ...
                stats.update(
                    {
                        x[0]: ([int(n) for n in x[1].split("-")] if "-" in x[1] else int(x[1]))
                        for x in zip(key, values)
                        if x[1] != "0"
                    }
                )
